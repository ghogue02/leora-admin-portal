 ...
 */
export const VALID_PAYMENT_TERMS: readonly string[] = [
  'C.O.D.',
  'Net 30 Days',
  'Net 15th of Next Month',
  'Net 30th of Next Month',
...
export function isValidPaymentTerm(paymentTerms: string): boolean {
  return VALID_PAYMENT_TERMS.includes(paymentTerms.trim());
}
...
export function calculateDueDate(
  ...
  try {
    ...
    if (terms === 'Net 15th of Next Month') {
      const dueDate = new Date(invoiceDate);
    ...
    if (terms === 'Net 30th of Next Month') {
      const dueDate = new Date(invoiceDate);
    ...
    // Used by: 1 customer
    if (terms === 'Net 45 Days') {
      const dueDate = new Date(invoiceDate);
      dueDate.setDate(dueDate.getDate() + 45);
      return dueDate;
    }

    // Unknown payment terms - default to C.O.D.
    // This should be caught in validation, but we handle gracefully
    console.warn(
      `Unknown payment terms: "${terms}". Defaulting to same-day payment (C.O.D.). ` +
...
export function calculateDueDateWithMetadata(
  ...
  const isDefaulted = !isValidPaymentTerm(terms);
...
export function validatePaymentTerms(paymentTerms: string): {
  ...
  if (!isValidPaymentTerm(terms)) {
    return {
      valid: false,
      error: `Invalid payment terms: "${terms}"`,
