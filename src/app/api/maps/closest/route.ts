...
export async function POST(request: NextRequest) {
  return withSalesSession(request, async ({ db, tenantId }) => {
    try {
      ...
      const { latitude, longitude, radiusMiles = 50, limit = 50 } = body;
      ...
      const customersWithDistance = customers.map((customer) => {
        ...
        return {
          id: customer.id,
          name: customer.name,
          address: [customer.street1, customer.street2].filter(Boolean).join(", "),
          city: customer.city || "",
      ...
      const nearbyCustomers = customersWithDistance
        .filter((customer) => customer.distance <= radiusMiles)
      ...
      console.error("[maps/closest] Failed to find nearest customers:", error);
      return NextResponse.json(
