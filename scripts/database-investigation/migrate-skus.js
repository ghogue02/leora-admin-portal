...
const lovable = (0, supabase_js_1.createClient)(LOVABLE_URL, LOVABLE_SERVICE_KEY);
// Paths
...
async function loadWellCraftedSkus() {
...
async function getExistingLovableSkus() {
...
function transformSku(wcSku, productMap) {
    // Map product ID
...
async function importSkuBatch(skus) {
...
async function createSkuUuidMap(wcSkus) {
    ...
    const lovableCodeMap = new Map();
    lovableSkus.forEach((sku) => {
